...
ETCD_HOST = os.getenv('ETCD_HOST', 'etcd')  # Customoizable
ETCD_PORT = int(os.getenv('ETCD_PORT', 2379)) # Customoizable

print(ETCD_HOST + ":" + str(ETCD_PORT))
# SCRIPT
...
def leader_election(etcd):
    """Leader election"""
    lease = etcd.lease(10)
    leader = etcd.get("/leader")  # Get the current leader
    if leader[0] is not None:  # Check if leader[0] is not None
        print(f"Leader already elected: {leader[0].decode('utf-8')}")
        return False
    if etcd.put_if_not_exists("/leader", NODE_ID, lease):
        print(f"Node {NODE_ID} has been elected leader.")
        return True
